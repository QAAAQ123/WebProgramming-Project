...
var UsersList = React.createClass({
	...
	render: function render() {
		return React.createElement(
			'div',
			{ className: 'users' },// 'users' 클래스명을 가진 div 생성
			React.createElement(
				'h3',
				null,
				' 온라인 상태 '// 온라인 상태라는 제목 생성(변경)
			),
			React.createElement(
				'ul',
				null,
				this.props.users.map(function (user, i) {// users prop을 순회하며 사용자 이름 리스트 생성
					return React.createElement(
						'li',
						{ key: i },// 고유한 key 할당
						user// 사용자 이름 표시
...
var Message = React.createClass({
    ...
    const messageClass = this.props.isOwnMessage ? 'message own-message' : 'message'; 
     // isOwnMessage prop에 따라 클래스명 결정(추가)
		return React.createElement(
			'div',
			{ className: messageClass }, // 클래스명을 messageClass로 설정(변경)
			React.createElement(
				'strong',
				null,
				this.props.user,
			...
			React.createElement(
				'span',
				null,
				this.props.text// 메시지 텍스트 표시
			),
      React.createElement(//타임스탬프(추가)
				'span',
				{ className: 'timestamp' },  // timestamp 클래스명을 가진 span 생성
				' (', this.props.timestamp, ')'// 메시지 전송 시간 표시
			)
		);
	}
});
...
var MessageList = React.createClass({
    ...
    const currentUser = this.props.currentUser; // 현재 사용자 정보 가져오기(추가)

		return React.createElement(
			'div',
			{ className: 'messages' },// 'messages' 클래스명을 가진 div 생성(변경)
			React.createElement(
				'h2',
				null,
				' 채팅방 ' // 채팅방 제목 생성
			),
			this.props.messages.map(function (message, i) { // messages prop을 순회하며 메시지 리스트 생성
				return React.createElement(Message, {
					key: i, // 고유한 key 할당
					user: message.user,// 메시지의 사용자 이름
					text: message.text,// 메시지 텍스트
          timestamp: message.timestamp, // 메시지 전송 시간(추가)
          isOwnMessage: message.user === currentUser // 현재 사용자가 보낸 메시지인지 여부(추가)
				});
			})
		);
	}
...
var MessageForm = React.createClass({
	...
	handleSubmit: function handleSubmit(e) {
		...
		var message = {//추가
			user: this.props.user,// 현재 사용자 이름
			text: this.state.text,// 메시지 텍스트
			timestamp: currentTime// 현재 시간
		};
		this.props.onMessageSubmit(message);// 메시지 제출 함수 호출
		this.saveMessageToLocalStorage(message); //메시지를 로컬 스토리지에 저장(추가)
		this.setState({ text: '' });// 입력 필드 초기화
	},

  //추가
	saveMessageToLocalStorage: function saveMessageToLocalStorage(message) {
		var storedMessages = JSON.parse(localStorage.getItem('chatMessages')) || [];// 로컬 스토리지에서 메시지 가져오기
		storedMessages.push(message);// 새로운 메시지 추가
		localStorage.setItem('chatMessages', JSON.stringify(storedMessages));// 메시지를 로컬 스토리지에 저장
	},

	changeHandler: function changeHandler(e) {
		this.setState({ text: e.target.value });// 입력된 텍스트로 상태 업데이트
	},

	render: function render() {
		return React.createElement(
			'div',
			{ className: 'message_form' },// 'message_form' 클래스명을 가진 div 생성
			React.createElement(
				'form',
				{ onSubmit: this.handleSubmit },
				React.createElement('input', {// 폼 제출 이벤트 핸들러 설정
					placeholder: '메시지', // 입력 필드 플레이스홀더
					className: 'textinput', // 클래스명 설정
					onChange: this.changeHandler,// 입력 필드 변경 이벤트 핸들러
					value: this.state.text// 입력 필드 값 설정
				}),
...
var ChangeNameForm = React.createClass({
	...
	getInitialState: function getInitialState() {
		return { newName: '' };// 초기 상태로 빈 문자열 설정
	},

	onKey: function onKey(e) {
		this.setState({ newName: e.target.value });// 입력된 텍스트로 상태 업데이트
	},

	handleSubmit: function handleSubmit(e) {
		e.preventDefault();// 기본 폼 제출 동작 방지
		var newName = this.state.newName;// 새로운 이름 가져오기
		this.props.onChangeName(newName);// 이름 변경 함수 호출
		this.setState({ newName: '' });// 입력 필드 초기화
	},

	render: function render() {
		return React.createElement(
			'div',
			{ className: 'change_name_form' },// 'change_name_form' 클래스명을 가진 div 생성
			React.createElement(
				'form',
				{ onSubmit: this.handleSubmit },// 폼 제출 이벤트 핸들러 설정
				React.createElement('input', {
					placeholder: '변경할 아이디 입력',// 입력 필드 플레이스홀더
					onChange: this.onKey, // 입력 필드 변경 이벤트 핸들러
					value: this.state.newName// 입력 필드 값 설정
				})
			)
		);
	}
...
var ChatApp = React.createClass({
	...
	getInitialState: function getInitialState() {
		return { users: [], messages: [], text: '' };// 초기 상태 설정
	},

	componentDidMount: function componentDidMount() {
		socket.on('init', this._initialize);// 소켓 초기화 이벤트 핸들러 설정
		socket.on('send:message', this._messageRecieve);// 메시지 수신 이벤트 핸들러 설정
		socket.on('user:join', this._userJoined);// 사용자 입장 이벤트 핸들러 설정
		socket.on('user:left', this._userLeft);// 사용자 퇴장 이벤트 핸들러 설정
		socket.on('change:name', this._userChangedName);// 사용자 이름 변경 이벤트 핸들러 설정

    //추가
    var storedMessages = JSON.parse(localStorage.getItem('chatMessages')) || [];// 로컬 스토리지에서 메시지 가져오기
    this.setState({ messages: storedMessages });// 상태에 저장된 메시지 설정
  },

	_initialize: function _initialize(data) {
		var users = data.users;// 사용자 목록
		var name = data.name;// 사용자 이름
	...
	_messageRecieve: function _messageRecieve(message) {
		var messages = this.state.messages;// 현재 메시지 목록

		messages.push(message);// 새로운 메시지 추가
		this.setState({ messages: messages });// 상태 업데이트
	},
	...
	handleMessageSubmit: function handleMessageSubmit(message) {
		...
		messages.push(message);// 새로운 메시지 추가
		this.setState({ messages: messages });// 상태 업데이트
		socket.emit('send:message', message); //메시지를 서버로 보냄
	...
	handleChangeName: function handleChangeName(newName) {
		...
		var oldName = this.state.user;// 현재 사용자 이름
		socket.emit('change:name', { name: newName }, function (result) {
			if (!result) {
				return alert('There was an error changing your name');// 오류 발생 시 알림
			}
			var users = _this.state.users;// 현재 사용자 목록

			var index = users.indexOf(oldName);// 현재 사용자 이름의 인덱스
			users.splice(index, 1, newName);// 새로운 이름으로 교체
			_this.setState({ users: users, user: newName }); // 상태 업데이트
		});
	},

  //추가
  handleClearMessages: function handleClearMessages() {
		localStorage.removeItem('chatMessages');  // 로컬 스토리지에서 메시지 제거
		this.setState({ messages: [] }); // 상태에서 메시지 제거
	},// 메시지 전체 삭제 기능
	...
	render: function render() {
		return React.createElement(
			...
			React.createElement(
				'div',
				{ className: 'center' },// 'center' 클래스명을 가진 div 생성
				React.createElement(UsersList, {
					users: this.state.users// UsersList 컴포넌트에 사용자 목록 prop 전달
				}),
				React.createElement(ChangeNameForm, {
					onChangeName: this.handleChangeName // ChangeNameForm 컴포넌트에 이름 변경 이벤트 핸들러 prop 전달
				}),
				React.createElement(MessageList, {
					messages: this.state.messages,// MessageList 컴포넌트에 메시지 목록 prop 전달
          currentUser: this.state.user // 현재 사용자 정보 prop 전달
				}),
				React.createElement(MessageForm, {
					onMessageSubmit: this.handleMessageSubmit,// MessageForm 컴포넌트에 메시지 제출 이벤트 핸들러 prop 전달
					user: this.state.user// 현재 사용자 정보 prop 전달
				}),
        //추가
        React.createElement(
					'button',
